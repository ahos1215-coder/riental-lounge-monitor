...
入力不足・例外時は入力をそのまま返す“無害な no-op”を徹底する。
"""
...
def same_slot_stats(history_df: pd.DataFrame | None, freq_min: int) -> dict[tuple[int, int], tuple[float, int]]:
    """履歴の実測から {(hour, minute): (max_total, n_nights)} を作る。
    ...
    except Exception:  # noqa: BLE001 — 後処理は予測を壊さない
        return {}
    if frame.empty:
        return {}
...
def late_night_clamp(
    ...
    *,
    ...
    if headroom < 0:
        headroom = 1.3
    stats = same_slot_stats(history_df, freq_min)
    if not stats:
        return points, 0
    freq = max(1, int(freq_min))
    ...
    for p in points:
        ...
        if not _is_late_night(ts):
            out.append(q)
            continue
        key = (int(ts.hour), (int(ts.minute) // freq) * freq)
        stat = stats.get(key)
        if stat is None:
            out.append(q)
            continue
        max_total, n_nights = stat
        if n_nights < CLAMP_MIN_NIGHTS:
            out.append(q)
            continue
        cap = headroom * max_total
        total_f = float(total)
        if total_f > cap:
