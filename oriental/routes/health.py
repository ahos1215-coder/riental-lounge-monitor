...
# 2026-09-26: 30 秒 → 120 秒。Supabase は問い合わせ1回ごとにログを1件残し、その量が無料プランの
# ログ取り込み枠（月1GB、2027年初めから適用）に数えられる。Render のヘルスチェックのように
# 数秒おきに叩く相手がいると、30 秒では1日 2,880 回の問い合わせになる（120 秒なら 720 回）。
# 収集は5分おきで、stale の判定もそれ以上の単位なので、2 分の遅れは検知時間に影響しない。
# 0 で無効化できる。
_DEFAULT_FRESHNESS_TTL_SEC = 120.0
...
#     約24秒かけて必ずここに落ちる**。しかも結果は TTL キャッシュ（既定 120 秒）に乗るため、
#     瞬断1回で 503 が最大 TTL 秒続く。これで再起動されるとサイトが落ちる ——
...
def _data_freshness(cfg: AppConfig) -> dict:
    ...
    結果は既定 120 秒の TTL でキャッシュする（`/healthz` はレート制限の対象外なので、
    毎リクエスト上流に問い合わせると連打で上流と egress とログを増幅できてしまうため）。
