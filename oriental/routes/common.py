...
def client_ip() -> str:
    ...
    注意（2026-08-21 第3ラウンドで訂正。意図した仕様の理解自体は変わらない）: 通常の利用者
    トラフィックは Vercel の Next.js プロキシ経由で来るが、Vercel Functions の送信元 IP は
    既定で**動的IPプール**であり、単一の固定 egress IP ではない（固定IPは有料アドオンで、
    無料枠運用のこのサイトでは使っていない）。旧記述「利用者全員が Vercel の egress IP という
    1つのバケットを共有する」は誤りで、実際は「Vercel の送信元ごとに分かれた少数のバケットを
    共有する」——単一バケットより粒度が細かい分、この訂正は安全側（より公平）に働き、
    レート制限の機能に欠陥は無い。それでよい。ここで守りたいのは「Backend を直叩きして
    高コスト API を連打される」ケース（外部レビュー F4）で、その直叩き側は各自の実 IP で
    別バケットになる。共有バケット側が詰まらないよう、上限は実トラフィック（CDN warming
    1周 約134リクエスト・分散済み）に対して十分な余裕を取ってある。
    """
