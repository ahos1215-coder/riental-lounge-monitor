...
- Night window（19:00-05:00）の判定・絞り込みは **店舗プレビュー UI** ではフロント専任（`frontend/src/app/hooks/useStorePreviewData.ts`）。
- **LINE ブログ下書き**は `frontend/src/app/api/line/route.ts` → **`insightFromRange.ts`** で同じ夜窓定義をサーバー側に適用（取得済み `/api/range` 行へのクライアント集計）。**Flask の `/api/range` に時間フィルタを足さない**（DECISIONS と整合）。
...
- [ONBOARDING] 初回の状況整理・方針確認・関連ファイルの読み込み（`plan/RUNBOOK.md` 冒頭のオンボーディング）
