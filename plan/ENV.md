...
## 0. 基本方針
- 本番・開発ともに **環境変数で挙動を切り替える**。
- 現在の優先度
  1. Supabase での 38 店舗自動収集（DATA_BACKEND=supabase がデフォルト）
  2. バックエンドも Supabase ログ前提に揃える
...
- `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` … 互換: `SUPABASE_SERVICE_KEY`（読み取り用）
- `STORE_ID` … Supabase 利用時の既定 store_id。クエリ `store` / `store_id` で上書き可。互換キー `SUPABASE_STORE_ID` も読まれる。
- 予測 API
  - `ENABLE_FORECAST`=1 で /api/forecast_* を有効化（0 のとき 503 相当）
  - `FORECAST_FREQ_MIN` … デフォルト 15（予測ポイント間隔）
  - `NIGHT_START_H` / `NIGHT_END_H` … 夜間ウィンドウ

---

## 1. どこで環境変数を使っているか
1. **Flask バックエンド**
   - `oriental/config.py`, `oriental/routes/data.py`, `oriental/routes/forecast.py`
   - `/api/range`, `/api/forecast_*` の挙動に影響
2. **マルチ店データ収集スクリプト**
   - `multi_collect.py`
   - 38 店舗を巡回して Supabase に書き込む
3. **必要ならフロントエンド**
   - `NEXT_PUBLIC_API_BASE_URL` など（現時点では未使用）
...
- WINDOW_START / WINDOW_END: 夜間ウィンドウ（予測・レンジ取得のデフォルト期間）
...
```
- DATA_BACKEND: `supabase` がデフォルト。`legacy` の切り替えも可能。
...
- MAX_RANGE_LIMIT: `/api/range?limit=...` の上限をクランプ
...
GAS_MAX_RETRY=3
```
- Supabase が有効な場合、GAS は OFF 推奨。ENABLE_GAS=1 で旧経路も利用可。
- 天気は **都道府県単位でキャッシュ** する実装。店舗定義 `STORES.pref` → `PREF_COORDS`（県代表座標）→ 無ければ `WEATHER_LAT/LON` の順で座標を決定し、pref ごとに 1 回だけ Open-Meteo を叩いて 38 店舗へ使い回す。
...
以上、環境変数を変えたらこのファイルも更新して「次の運用者」に引き継いでください。
