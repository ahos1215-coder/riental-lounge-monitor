...
???????????
??	?	??	??
store	string	??	Supabase ???????????????????(store_id)????
store_id	string	??	Supabase ?????????store ?????????
from	string	??	???(YYYY-MM-DD/ISO8601)?????????
to	string	??	???(YYYY-MM-DD/ISO8601)?????????
limit	int	??	?????0 ??????MAX_RANGE_LIMIT ??????
order	string	??	"asc" / "desc"????? "asc"?
tz	string	??	??????????? Asia/Tokyo?

Data source

DATA_BACKEND=supabase: Supabase logs (/rest/v1/logs) ??? (SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY ??)
DATA_BACKEND=legacy: ???? JSON + GAS ?????????

Supabase 経路では logs テーブルの weather_code / weather_label / temp_c / precip_mm も取得して rows に含める（store_id クエリ指定時も同様）。
...
rows ????????: ts, men, women, total, store_id, src_brand, weather_code, weather_label, temp_c, precip_mm????????????????
...
Data source: DATA_BACKEND=supabase ?? Supabase logs (/rest/v1/logs) ???????SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY ????DATA_BACKEND=legacy ?? GoogleSheet + ???? JSON ???????
...
????????
??	?	??	??
store	string	??	???????Supabase ?? store_id ??????????????? store_id?
...
????????????: ts, men_pred, women_pred, total_pred
...
??????: /api/forecast_next_hour ???? DATA_BACKEND ? supabase/legacy ??????
...
????: store/store_id, data_backend, supabase.{url,service_role,store_id}, timezone, window.{start,end}, http_timeout, http_retry, max_range_limit
