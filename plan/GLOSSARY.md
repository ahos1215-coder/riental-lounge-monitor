...
| **Fan-in Matrix** | Weekly Report の GHA 手動実行（緊急用）の構成。Fan-out（オリエンタル37店舗並列）→ Fan-in（Artifact 集約・index.json 再構築・Git commit 1回）。通常運用のローカル生成はこの構成を使わず単一プロセスで全42店舗を順次処理する。 |
