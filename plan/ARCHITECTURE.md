...
- 店舗数: **42**（Oriental Lounge + ag 37 + 相席屋 5）。**2026-07-11 sapporo_ag閉店により43（38+5）から42（37+5）に減**。正本は `oriental/utils/stores.py` の `ALL_STORE_IDS` と `frontend/src/data/stores.json`（行数一致必須）。
...
- **Oriental Lounge + ag (37店舗、2026-07-11 sapporo_ag閉店で38→37)**: `oriental-lounge.com/` トップページから 1 リクエストで全店舗の人数を SSR 抽出 (`src_brand="oriental"`)
...
- 1 サイクル合計 **2 リクエストで全 42 店舗**を収集。リクエスト数 97% 削減 (旧: 38 個別リクエスト)
...
Weekly Report（毎週水曜 06:30 JST）:
  ...
  └─ scripts/run_weekly_local.ps1 -Stores all
     └─ generate_weekly_insights.py --stores all（INSIGHTS_LLM_BACKEND=ollama, gemma4:e4b）
        全42店舗を単一プロセスで順次処理（Fan-in Matrix は使わない）
...
ML Model Training（毎日 05:30 JST・毎週月曜 07:00 JST — GHA schedule。これは移行対象外、引き続き GHA が本流）:
  ...
  └─ scripts/train_ml_model.py
     ...
     ├─ `oriental/utils/stores.py` の ALL_STORE_IDS（42店舗）を allow-list に LightGBM モデル学習
...
- `oriental/routes/data.py`（**B8で分割済み**。Blueprint `bp` の定義 + `/`（index）のみを持ち、末尾の副作用importで下記2モジュールのハンドラを同じ`bp`に登録する集約ポイント）
- `oriental/routes/data_range.py`（/api/current, /api/range, /api/range_multi（ThreadPoolExecutor 並列化）— TTL+single-flightキャッシュ、`RANGE_CACHE_TTL`/`RANGE_CACHE_WAIT_TIMEOUT`/`RANGE_CACHE_MAX_ENTRIES`）
- `oriental/routes/data_meta.py`（/api/meta, /api/holiday_status, /api/second_venues）
- `oriental/routes/forecast.py`（Blueprint `url_prefix="/api"`。/api/forecast_next_hour, /api/forecast_today, /api/forecast_today_multi, /api/megribi_score — ThreadPoolExecutor 並列化・TTL+single-flightキャッシュ）
- `oriental/routes/forecast_accuracy.py`（**B8で forecast.py から分離**。/api/forecast_accuracy, /api/forecast_snapshot。`_fetch_live_accuracy` はここに定義）
...
- `oriental/ml/forecast_service.py`（ML 推論オーケストレーション。今夜アンカー補正（`FORECAST_ANCHOR_TONIGHT`/`FORECAST_ANCHOR_DECAY`）+ 採点由来ブレンド重み（`FORECAST_BLEND_WEIGHTS_TTL`）を含む）
...
- `oriental/ml/postprocess.py`（予測後処理。ベースラインブレンド（`FORECAST_BASELINE_BLEND`）+ 後段クランプ（`FORECAST_LATE_CLAMP`、曜日バケット対応 `FORECAST_CLAMP_DOW_AWARE`））
...
- `oriental/utils/stores.py`（**B7で `frontend/src/data/stores.json` からの導出に変更**。`_load_store_ids_from_stores_json()` が `stores.json` の `brand` フィールドから `STORE_IDS`/`AISEKIYA_STORE_IDS` を導出し `ALL_STORE_IDS` を組む。読み込み失敗時のみファイル内 `_FALLBACK_STORE_IDS`/`_FALLBACK_AISEKIYA_STORE_IDS` にフォールバック。store 解決・ML の allow-list の正本。**2026-07-11 sapporo_ag閉店で 37+5=42店**）
- `multi_collect.py`（収集ロジック。Oriental Lounge トップページ + 相席屋トップページの2リクエストで全42店舗を取得）
...
- `frontend/src/app/config/stores.ts`（42 店舗マスタ。`stores.json` を brand フィルタ無しでそのまま読み込む）
