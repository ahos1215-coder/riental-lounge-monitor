...
Last updated: 2026-05-05 (Phase 1 ✅ / Phase 3 ✅ / Phase 4 ✅ / Phase 5 ⚠️ 置換 / Phase 6 → 別計画書へ / Phase 2 のみ未着手)
Status:
...
- ✅ **Phase 4 (フォールバック改善)** 実装済 (`8d72a8b`, 2026-05-05) — `buildFallbackBlogDraftMdx()` を Phase 1 と同じ「自然文 80-160 字、見出し / 箇条書きなし、です・ます調」に書き換え。edition 別に分岐 (evening_preview = 推量形、late_update = 現況観察、18:00 予測には言及せず)。dayLabel + secondary_wave も自然文に折り込む
...
### Phase 4: フォールバック改善 (工数 半日) ✅ 実装済 (`8d72a8b`, 2026-05-05)
**対象**: `buildFallbackBlogDraftMdx()` in `draftGenerator.ts`

実装内容: 当初構想 (「観測データのみ + コメント休止宣言」) より一歩踏み込み、Phase 1 と同じ自然文スタイルで生成するように書き換えた。これにより 429 が起きてもユーザー視点では他店舗との一貫性が保たれる。

- [x] 旧 `## 今日の結論 + 箇条書き` を撤廃
- [x] 80-160 字の自然文 1-2 文に変更 (`です・ます調`)
- [x] edition 別に分岐:
  - `evening_preview` (18時): 「○曜の今夜は <peak> 前後がピークになりそうで、混雑度の目安は <crowd> です。」
  - `late_update` (21:30): 「○曜の現時点の混雑度は <crowd>、ピーク帯は <peak> 前後の流れです。」 (18:00 予測には言及しない、decoupled editions の方針通り)
- [x] dayLabel と secondary_wave があれば自然文に折り込む
- [x] frontmatter のキーは Phase 1 と完全互換 (categoryId / level / store / facts_id / facts_visibility)
- [-] 「フォールバック生成」の明示は採用しなかった (Claude 案より): ユーザーに技術的事情を伝えるより一貫した読み物として読める方が体験が良いと判断
