...
- **定時ブログ（本番）**: `.github/workflows/trigger-blog-cron.yml`（全店舗・matrix）。**失敗店舗のみの再実行**は `.github/workflows/retry-blog-draft-stores.yml`。**一部店舗失敗時の Slack/Discord**（`OPS_NOTIFY_WEBHOOK_URL` 設定時）は `trigger-blog-cron.yml` 内の **`summarize-blog-matrix`** → `notify-partial-blog-failures`。集計は **ジョブ結論に加え、各 matrix ジョブの steps**（GitHub API）を見て、`continue-on-error` でジョブが緑でも **GET ステップ失敗**（504 等）を拾う。成否の正本は Supabase `blog_drafts`（運用要約はリポジトリ直下 **`STATUS.md`**、`plan/BLOG_CRON_GHA.md`）。将来の非同期化メモは **`plan/BLOG_CRON_ASYNC_FUTURE.md`**。
