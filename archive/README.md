...
- `next_prototype_2025/`（`app/page.tsx` / `app/api/forecast/route.ts` / `tailwind.config.js`）— 2025年の初回コミットで入った Next.js プロトタイプ。長崎店固定・`http://127.0.0.1:5000` 固定で、**現役のフロントは `frontend/` 配下**。初回コミット以降1度も変更されず、ビルド対象でも参照元でもなかったが、リポジトリ直下にあるため初見の人間/AI が現役実装と誤認していた（2026-08-21 外部レビュー）。削除ではなく退避（旧 リポジトリ直下 `app/` と `tailwind.config.js`）。
