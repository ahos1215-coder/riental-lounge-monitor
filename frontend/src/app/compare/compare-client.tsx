...
// Recharts (+d3) は First Load JS への影響が大きい（gzip 換算で ~113KB）。
// グラフは店舗を2件以上選択した後にしか表示されないため、店舗詳細ページの
// PreviewMainSection と同じパターンで ssr:false + 遅延ロードにする。
const CompareChart = dynamic(() => import("./CompareChart"), {
  ...
  loading: () => (
    <section className="mt-8 rounded-2xl border border-white/10 bg-black/40 p-4 md:p-6">
      ...
      <div className="mt-4 flex h-72 w-full items-center justify-center">
  ...
  return (
    <main className="relative min-h-[calc(100vh-80px)] bg-black text-white">
      <div className="relative mx-auto w-full max-w-5xl px-4 pb-16 pt-10">
        ...
        {/* Merged chart（Recharts は選択後にのみ必要 → 遅延ロード） */}
