...
// （既存の import 元を壊さないため）。
import { buildStoreFullName, type StoreMeta } from "../config/stores";
import { parseRangeEnvelope } from "@/lib/range/rangeRows";

// ---- re-export: 型の実体は frontend/src/lib/forecast/types.ts へ移設した ----
// （lib → app/hooks の逆転依存と nightWindow との型循環 import を解消するため。
//  既存の import 元（このバレル経由で型を取っているファイル）は無改修で動く）
export type {
  PreviewRangeMode,
  TimeSeriesPoint,
  ForecastStatus,
...
/** 封筒（配列 / {rows} / {data}）をほどき、ts を持つ行だけを通す。 */
export function parseRangePoints(raw: unknown): RangePoint[] {
  return parseRangeEnvelope(raw).filter(isRangePoint);
}
