...
日付境界について（2026-09-06 に日次側の対象日マージンを直したときの確認結果）:
このスクリプトは **対象日という概念を持たない**。「今から何日前の更新か」という相対年齢を
MAX_STALENESS_DAYS（既定8日）と比べるだけなので、GHA の cron が何時間遅れても判定は
連続的にしか変わらない（日付が跨いだ瞬間に対象がずれる、という穴が無い）。週次生成は
7日周期で基準が8日なので、遅延の許容幅は常に約1日ある。
よって check_daily_published.py で入れた「JST -18時間」のマージンは、ここには不要。
