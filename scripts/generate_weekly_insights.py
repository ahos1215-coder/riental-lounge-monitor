...
def _generate_ai_commentary(
    *,
    ...
    system_instruction = (
        ...
        "営業文句 (ぜひお越しください等)。挨拶。\n\n"
        "■ 構成 (必ずこの形式)\n"
        "1. リード文 1 行 (40-60 字程度): 先週/来週の最大の特徴を 1 文でまとめる\n"
        "2. 改行を 1 つ挟む\n"
        "3. Markdown 箇条書き 3〜5 項目 (`- ` で始まる行): 各項目は曜日 / 時間帯 + 数値 + 簡潔な解釈\n"
        "例:\n"
        "先週は週末を中心に高い混雑が見られました。\n"
        "\n"
        "- 金曜・土曜: 平均 60% 台、ピーク時は満席に近い時間帯あり\n"
        "- 火曜: 平日ながら平均 62%、ピーク 100% と週末並みの賑わい\n"
        "- 月曜・水曜: 平均 16-24% と比較的落ち着いた状況\n"
        "- 5/3 (日): GW 入りで日曜にしては高稼働、平均 66%\n\n"
        "■ 内容ガイド\n"
